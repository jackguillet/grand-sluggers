...
- Load `docs/agent-rails.md` (#647) and `data/agent/debug-protocol.json` for the kind you are in. Append a signature when you repair a novel failure, in the same PR as the fix. On the second firing, promote it to a test. A procedural lesson grows `.grok/skills/character-art/` or the spec, not only the PR body.
