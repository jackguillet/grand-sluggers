...
| --- | --- | --- | --- | --- |
| Status volume | a body inside it, for a stated time | `freeze_volume`, `lava_pit`, `fire_breath` | landing point in the disc slows **every** chaser for the **whole** play (×0.45) and adds a 0.4 drop roll | per-body touch test in the tick, a duration, CPU route cost, a tell on the body; the drop roll's fate is FD-08-R1 |
...
| "A play is decided by geometry, never by a roll or a caption" | AGENTS.md; roadmap Phase P ban on a `Random` that produces a `PlayEvent` | **FD-08 C makes an exception for park hazards only.** Not yet written into AGENTS.md: the wording depends on FD-08-R1. Every other play keeps the rule. |
...
FD-01 to FD-09, FD-12, FD-16 and FD-17 are **DIRECTION ACCEPTED**. The other 7 are **OPEN**, with refinement FD-08-R1. The recommendation is the author's proposal. Only Jack's recorded answer selects an option. Each acceptance line is a proposed falsifier, not a passed gate. Source ids resolve in the [research report](research-fields.md#sources).
...
**Refinement FD-08-R1 — OPEN: may a random hazard draw decide a play's result directly, or only what the hazard does?** **1 — random world, geometric result:** a draw decides when a hazard fires, which exit it picks, where it throws the ball; the ball and the bodies then decide the play. No out, hit, drop or catch is awarded by chance. **2 — draws may also decide results:** a slowed fielder drops the catch 40 % of the time (today's `drops.frozen`), a plant eats the ball or does not. Recommendation: 1. Today both kinds exist: the warp exit is a draw on the world (`Fielding.cs:701`); `drops.frozen` is a draw on the result (`Match.cs:702`).

**Decision — Jack, September 21, 2026: C.** "C. we can add random party elements for the hazards." Park hazards may carry random party elements: permission, not a requirement. The author's recommendation was A. What follows as engineering, not as a tradeoff: every draw comes from the match's seeded stream and is a typed live event, so the same seed replays the same game; the CPU gets no foresight of a draw; Harbor stays the park with no hazard. **This conflicts with the standing rule "a play is decided by geometry, never by a roll"** (AGENTS.md; roadmap Phase P ban). The exception is scoped to park hazards only; its wording waits for FD-08-R1. Which hazards are random, how often, and every number stay open. Full provenance is in the canonical JSON.
