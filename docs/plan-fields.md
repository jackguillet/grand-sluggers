...
| FR-10 | **Measure every park against Harbor.** ✅ #828 (PR #834): `cli match --cohort park-factors` reports run and home-run factors on predeclared seeds, both roots, day and night, for every park in the catalog; `tools/park-factors.py` is superseded and kept only as the tool that measured the `d0c6e12c` baseline. Each park declares its intent before it gets numbers. | Harbor cohorts (F693); "no expectation edited to make it pass" | A park PR carries the report. A factor outside the accepted band (FD-02) is a finding, not a tuning target for that PR. `SF-30` (`ParkFactorsCohortTests`) falsifies the *report* — determinism, every catalog park, the control at 1.00, day and night, the root named — and asserts no factor, so the cohort can never quietly become a gate. |
