...
| Hazards on or off | always on | — | a match option, default on (an empty instance list) | FD-10 ✅ |
| Night | one park number + two code rules | `Match.Night` | declared night block, validated like the day block | FD-11 ✅ |
...
FD-01 to FD-14, FD-16, FD-17 and FD-19 are **DIRECTION ACCEPTED**. FD-15 and FD-18 are **OPEN**. The recommendation is the author's proposal. Only Jack's recorded answer selects an option. Each acceptance line is a proposed falsifier, not a passed gate. Source ids resolve in the [research report](research-fields.md#sources).
...
**Decision — Jack, September 21, 2026: A.** Reply "a". The CPU reads the same resolved park the ball flies in: the route planner costs status volumes and solid bodies and plans with the ground's start and brake multipliers; runner margins and the CPU batter use the park's air, ground and fence; outfield placement follows the park's depth. **No foresight of a random draw** (FD-08): the CPU sees the typed event when the human does. Every hazard pattern has a CPU-seat and human-seat scenario pair. The difficulty ladder keeps its multipliers; whether a rung reads hazards less well is not selected. Full provenance is in the canonical JSON.
