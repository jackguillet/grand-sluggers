...
- **On demand (Actions → Full tests):** every test class, the C80 re-run, the tool tests and the evidence seals. The `balance_only` input runs only the `[Trait("Kind","Balance")]` set. Run it only when Jack says he wants to balance the game. No PR owes it, a tuning PR included. Contract: [agent-rails.md](agent-rails.md) §1.2.
