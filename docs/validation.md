...
Data conventions are validators, not habits. `cli art` refuses a number in `data/` whose key spells its unit long (`Seconds`, `Feet`) or names a time or a speed with no unit (`smashFreeze`, `restSpeed`): end it in `Sec`, `Ft`, `Deg`, `FtPerSec` or `Mph` (`DataNaming`; the keys that predate the rule are its grandfather list, and a rename drops its line). A one-row catalog file is named for its id (`data/parks/harbor-diamond.json` holds `harbor-diamond`), checked when the catalog loads. Python reads a data file with `tools/jsonc.py`, the sim's own rules (comments, trailing commas); do not copy a reader. Tests name a park through `ParkId` (`Support/ParkId.cs`).
