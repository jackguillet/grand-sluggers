...
- **On demand (Actions → Full tests):** every test class, the tool tests and the evidence seals (`game-feel-flight-probes --check`, `pitch-family-probes --check`, `compact-field-report.py --check`). A feature PR does not reseal them; a balance pass does. The `balance_only` input runs only the `[Trait("Kind","Balance")]` set. Run it only when Jack says he wants to balance the game. No PR owes it, a tuning PR included. Contract: [agent-rails.md](agent-rails.md) §1.2.

## Portable checks

These run from a clean checkout on macOS or Linux and do not require Unity or a checkout-specific `Library/`. The `dotnet test` line below is the full suite: CI runs it in Full tests; do not run it on the shared Mac:
...
Data conventions are validators, not habits. `cli art` refuses a number in `data/` whose key spells its unit long (`Seconds`, `Feet`) or names a time, a speed or a length with no unit (`smashFreeze`, `restSpeed`, `radius`): end it in `Sec`, `Ft`, `Deg`, `FtPerSec` or `Mph`; a scale ends in `Mul` or `Scale`, and a rate such as `perFtOfHeight` names both units (`DataNaming`; the keys that predate the rule are its grandfather list, a row may name a whole folder as `parks/*.json`, and a rename drops its line). A one-row catalog file is named for its id (`data/parks/harbor-diamond.json` holds `harbor-diamond`), checked when the catalog loads. Python reads a data file with `tools/jsonc.py`, the sim's own rules (comments, trailing commas); do not copy a reader. Tests name a park through `ParkId` (`Support/ParkId.cs`).
...
Launch the pinned GUI editor with `-executeMethod GrandSluggers.EditorTools.AtBatInputGate.Run` (menu **Grand Sluggers → Verify At-Bat Input**), from a dedicated worktree. Set `GS_VALIDATION_REVISION` and optionally `GS_AT_BAT_INPUT_EVIDENCE`; no batch or quit flag. The gate creates two temporary InputSystem gamepads, drives actual Controls and director SET/Flight methods, then removes the devices and restores input state.

Cases cover normal tap, charged release, release after entering the pitcher's windup, ignored SET releases on the SET-to-Flight frame in both one-seat and two-seat play, camera-relative horizontal pitching, and the batter cursor remaining independent of pitch curve. This verifies routed synthetic input, not physical controllers, perceived feel, or a full human half.
