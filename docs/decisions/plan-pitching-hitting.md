...
Tracker: [#803](https://github.com/jackguillet/grand-sluggers/issues/803), serving #209 and coordinating #534. Session kind: **Gameplay research/documentation**. Baseline: `05471a6`. Research: [reference comparison](../archive/pitching-hitting/research-pitching-hitting.md). Canonical structured record: [pitching-hitting-decisions.json](../research/pitching-hitting-decisions.json). Implementation order, open contract items and the ledger: [plan-pitching-hitting-implementation.md](../archive/pitching-hitting/plan-pitching-hitting-implementation.md).
...
1. Discuss material tradeoffs using short player-facing examples. The broad direction round and recorded refinements stay accepted. **Next work: consolidate the agreed input/commitment and resource contracts, then bring back only unresolved choices that materially affect play.** Exact modifier bindings, resource deduction/cancel/foul handling, numerical trials and ability designs remain open. PH-16-R14 excludes plate-level chemistry. The Star-counter red flash (PH-16-R12) and small secondary fatigue steering penalty (PH-08-R1) remain candidates, not locked effects.
...
## Current agreed mechanics

- **Ordinary pitching:** every character has Fastball plus two assigned families from Changeup, Curveball, Slider and Sinker. RB/Tab cycles before charging, resets to Fastball each pitch and locks the ordinary family at charge start. Location comes from mound position and pitch shape, with bounded live steering. Charge trades power for steering correction while preserving characteristic movement. Velocity, Movement, Control and Endurance are separate attributes (PH-02–06, PH-15-R1–R6).
- **Ordinary batting:** batter-linked horizontal coverage, normal movement while loading, placement-led quality and timing-led hit direction. Contact improves spatial forgiveness; Power controls hitting strength. Charging trades spatial forgiveness for power without changing the timing window. Release commits; East/G cancels an uncommitted load, or a bunt hold replaces it and discards charge. Canceled inputs require fresh-press rearming (PH-09–13, PH-15-R5/R7).
- **Bunting:** LT/J holds toward third, RT/L toward first; latest press wins when both are held. Side can change until contact; releasing all holds withdraws. Contact is geometric without a separate swing press, and quality governs softness/control rather than ordinary exit-power reward (PH-14-R1–R5).
- **Fatigue:** effort-driven, per character, lasting for the match with no recovery or swap reset. Conceded hits/runs add no fatigue surcharge. The primary effect is reduced peak pitching power; a small secondary steering penalty remains tentative (PH-08-R1–R3).
- **Special actions:** one Star Pitch and one Star Swing per character, activated through a held modifier plus normal action, with special intent locked at action-button release. Ordinary charge tradeoffs remain. Star Pitches retain normal timing windows; Star Swings may change contact area by ability, with actual contact required. Highest-cost specials are captain-exclusive. Exact abilities and modifier bindings remain open (PH-16-R1/R2/R7–R11/R13).
- **Star resources and chemistry:** one pool per team, usable starting reserves, gains for both teams at completed plate appearances plus performance bonuses, and tiered ability costs. Missed Star Swings spend full cost. Insufficient Stars produce the ordinary action at intended timing, spend no Stars and give clear feedback; a red flash at the Star counter is a candidate. No plate-level chemistry bonus; existing fielding/teamwork chemistry remains (PH-16-R3–R8/R12/R14).

The same human assistance rules, legal CPU action limits, causal feedback and serial standalone acceptance requirements still apply (PH-17–20). D7 requires its named re-sit before pitch-speed tuning. The overview records directions, not coefficients or passed playtests; detailed provenance and unresolved contracts follow.
