...
**The gap now.** Phase P is still not done — agents do not pass the sittings — but the parity sitting **has been run** and **all six sitting children have landed**. Jack played a full Exhibition game on standalone `main` `95026535d3` (2026-09-13). Next human step is a **re-sit of #534** on current `main` (includes #672–#675, #678, #679) and the **D7 pace call** (spec: wait until that re-sit, #677). Then the narrated half and the #346 book sitting. Until those three gates pass, do not start Phase T.
...
**Sequence from here.** Rebuild the standalone, **re-sit #534** and call D7, narrated half, #346 book, one skeptic pass, then the toy playset (#246, Phase T). Phases A–E stay the presentation plan they always were.
...
| P4 #566 | #597 the roll is dead, one throw model, CPU fielder table · sitting: liner FlyOut (#666 / #674) · OF glove by the roll not the bounce (#667 / #678) · stand-up ring, dive at the rim (#669 / #679) |
...
| P8 #570 | #600 stamps from typed outcomes (#578) · #601 cameras per class, the catcher stays put (#574) · #602 the book follows the verbs · sitting: liner shot between hopper and fly (#665 / #675) |
...
1. **Parity sitting (#534)** — **played, not passed** (standalone `main` `95026535d3`, 2026-09-13, full Exhibition game). All six sitting children **shipped**. **Re-sit on current `main`** (includes #672–#675, #678, #679), then the **pitch-pace call (spec D7)** — wait until that re-sit (#677). Closes #563, #564 when Jack would keep the feel.
...
#### #534 sitting children (2026-09-13) — all landed

| Child | Observed | Status |
| --- | --- | --- |
| #665 | Line drive vs ground ball does not read HUD-off | ✅ #675 (liner shot between hopper and fly) |
| #666 | Catching a liner in the air must be an out | ✅ #674 |
| #667 | OF glove is nearest at the bounce, not earliest on the roll | ✅ #678 |
| #668 | Changeup is not noticeably different | ✅ #672 |
| #669 | Landing ring too wide; rim should dive | ✅ #679 |
| #670 | Swing release too late — `leadSec` earlier than the plate | ✅ #673 (`leadSec` 0.18) |

Also landed beside the sitting: plate bat beside the head (#560 / #671), turntable from silhouette height (#559 / #664), H = Call time / Esc = book (#629 / #663), fly-ball ground shadow code (#646; Jack's visibility gate #645 still open). D7 remains **wait** until the re-sit.
...
**Human gates inside Phase P.** After P1: the #534 parity sitting (played 2026-09-13; children shipped; re-sit then D7). After P5: one half-inning narrated (every out explained). After P7: three innings, then the #346 book sitting. Agents do not pass these.
...
**Re-sit #534 on current `main`, call D7, then the other two gates, then Phase T.**

1. `python3 tools/local-player.py` so the window is current `main` (changeup, `leadSec` 0.18, liner shot, liner FlyOut, OF-by-roll, rim dive).
2. **Re-sit #534** (pad and keyboard). File what still sticks. Then choose D7: keep / speed up / middle. Do not retune `arcadeScale` from the 2026-09-13 game.
3. Narrated half, then the #346 book sitting. Call time → How to play must be enough.
4. One skeptic pass: Exhibition → pitch → swing → grounder → throw → steal on a preview build; harness and S-29 (`cli match --trace`).
...
6. Phase D stays gated on "Exhibition is the reason people stay". Do not start R5.
