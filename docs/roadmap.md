...
**The gap now.** Phase P is still not done — agents do not pass the sittings — but the parity sitting **has been run**. Jack played a full Exhibition game on standalone `main` `95026535d3` (2026-09-13). Six children were filed. Three of those plus three older presentation notes have already landed (table below). Remaining on the parity path: liner vs hopper read (#665), OF route on the roll (#667), landing ring / dive (#669), and the **pitch-pace call (D7)**. Then the narrated half and the #346 book sitting. Until those three gates pass, do not start Phase T.
...
**Sequence from here.** Remaining #534 children (#667, #669, #665), re-sit parity and call D7, narrated half, #346 book, one skeptic pass, then the toy playset (#246, Phase T). Phases A–E stay the presentation plan they always were.
...
#647 is **code-complete except later R5**. Do not build more agent infrastructure. Use the rails on sitting children.
...
| P4 #566 | #597 the roll is dead, one throw model, CPU fielder table · sitting: liner held before the bounce is FlyOut (#666 / #674) |
...
1. **Parity sitting (#534)** — **played, not passed** (standalone `main` `95026535d3`, 2026-09-13, full Exhibition game). Keyboard + mouse, one pad, two pads: rubber walk with the aim ring, CHANGE, the visible SWAP pick, tap / MAX / overcharge / changeup / break / star, the taller oval and a strike at the top of the frame, MAX on release and PERFECT / NICE / SOUR only on contact, early pulls / late pushes, bunt pop and the two-strike foul bunt K, HIT BY PITCH, the CPU working corners and mistracking when you move on the rubber. Then the **pitch-pace call (spec D7)** — still uncalled. Closes #563, #564 when Jack would keep the feel.
2. **Narrated half-inning** — every out has a reason you saw: a 6-4-3 or a FIELDER'S CHOICE, the 3-6 tag and a rundown, a silent tag at third versus a bang-bang icon, the relay home on a tag-up, the batter running through first. Watch the outfield on every fly for #580 and the glove read (#609). Closes #565–#568 and the fielding notes. **Not run yet.**
3. **Three innings, then the book (#346)** — pad only, normal difficulty, then keyboard + mouse; Call time → How to play must be enough. Closes #209 and #342. **Not run yet.**

What sticks becomes a sitting-found child. Distill: file the child, append `data/agent/debug-protocol.json` in the fix PR, promote on the second firing.

#### #534 sitting children (2026-09-13)

| Child | Observed | Status |
| --- | --- | --- |
| #665 | Line drive vs ground ball does not read HUD-off | open (P8) |
| #666 | Catching a liner in the air must be an out | ✅ #674 |
| #667 | OF glove is nearest at the bounce, not earliest on the roll | open (P4 / D16) |
| #668 | Changeup is not noticeably different | ✅ #672 |
| #669 | Landing ring too wide; rim should dive | open (P4 + dive take) |
| #670 | Swing release too late — `leadSec` earlier than the plate | ✅ #673 (`leadSec` 0.18) |

Also landed beside the sitting (not #534 children, same week): plate bat beside the head (#560 / #671), turntable from silhouette height (#559 / #664), H = Call time / Esc = book (#629 / #663), fly-ball ground shadow code (#646; Jack's visibility gate #645 still open).
...
**Human gates inside Phase P.** After P1: the #534 parity sitting (played 2026-09-13, not passed — remaining children #665 #667 #669, then D7). After P5: one half-inning narrated (every out explained). After P7: three innings, then the #346 book sitting. Agents do not pass these.
...
**Finish the #534 children, re-sit parity, then the other two gates, then Phase T.**

1. Remaining parity children (one worktree each, session kind on the issue): **#667** OF route on the roll (gameplay, D16), **#669** ring / dive (gameplay then art), **#665** liner vs hopper read (presentation). Do not start R5.
2. Rebuild the standalone (`python3 tools/local-player.py`) and **re-sit #534** on the revision that includes #672 / #673 / #674. Make the **D7 pace** call. File what still sticks.
3. Narrated half, then the #346 book sitting. Pad and keyboard. Call time → How to play must be enough.
4. One skeptic pass: an agent plays Exhibition → pitch → swing → grounder → throw → steal on a preview build and reruns the harness and S-29 (`cli match --trace`).
5. **Phase T (#246)** only after those gates: toy language (#247), front of house as a carnival (#248), lineup as the chemistry toy (#249), in-play cartoon juice (#250). Each closes on a still you would show a friend.
6. Phase D stays gated on "Exhibition is the reason people stay".
