...
### Covering bodies join the one movement profile — `9fb972e` / #709

Research first. Cover, cutoff and backup all move through `LivePlaySystem.Field.StepFlat` at a flat `cover.ftPerSec` of 28 ft/s — no acceleration, no braking, no analog shaping, no character difference — beginning at `max(cover.startSec .23, the position's reaction lockout)` and stopping dead within `stopFt` 1.2 feet. Ball pursuit is the accepted profile at 18 ft/s with a .20-second build-up and a .10-second brake. So the same body ran **56% faster to a bag than to a ball**, started instantly and stopped dead, which F693-02-pursuit-speed's one-profile direction did not intend.

Second base is the pressure point: the middle infielders start 38 feet from it while the corner fielders stand almost on their bags, so cover arrival on C80 was .54 / 1.38 / .54 seconds for first, second and third at the flat speed, against .59 / 1.89 / .59 under the accepted profile without a read and .84 / 2.14 / .84 with one. On the double-play feed — shortstop fields and flips 38.2 feet, .73 seconds from the command — the accepted profile **with** its .25-second read breaks the quick pivot by .16 seconds, while **without** a read it leaves +.39 seconds on an average ball and +.09 on a quick one.

Three things turned out not to be at risk. The **cutoff** repositions at cover speed while a fly hangs 4.3 to 8.5 seconds, so even at 18 ft/s its deepest move is about two seconds and it is never the late one. The **relay** still beats the deepest C80 direct throw, the centre-field wall to home at 280 feet taking 4.80 seconds direct against 4.00 for two 140-foot legs with a buffered command; the control field's 400-foot equivalent is 10.20 seconds. And **receiver reach is now consistent**: `cover.radiusFt` is 6 feet and the accepted stand-up catch reach is now also 6, where before the reach decision a receiver had 6 feet against a fielder's 13. That harmonisation was incidental and should not be split apart again.

Jack accepted F693-02-coverage-budget on September 15, 2026, choosing to inherit the profile without the read. Cover, cutoff and backup use **18 ft/s with the .20-second build-up, the .10-second brake and character speed differences**, beginning **at contact with no read**; `cover.startSec` and the position reaction lockout no longer gate a covering body. Dropping the read is justified rather than convenient, since covering is a known assignment to a fixed spot while the read models recognising where a batted ball went. **The accepted pursuit reads are unchanged**, and a covering body that stops covering and starts chasing a live ball takes the ordinary read. `cover.stopFt`, `cover.backupFt` and `cover.radiusFt` are retained.

**Correction on the record.** An earlier option summary said a Snap Throw would tighten the double-play feed further. It would not. Snap Throw is a .22-second release after cleanly *receiving a teammate throw*, so it does not apply to the feed, which is thrown by the fielder who fielded a batted ball; it applies to the pivot onward to first, by which point the cover has arrived. The case where a Snap Throw can genuinely outrun a walking cover is a **relay**, a cutoff receiving and snapping onward to a bag whose cover is still moving. That is queued as F693-02-throw-before-cover rather than assumed, alongside the named asymmetry that `CpuThrowReadySec` makes the CPU wait for its receiver while a human seat can fire at an empty bag with `cover.radiusFt` alone deciding the catch.

Validation: report/check and whitespace pass; the checker rebuilds every feed margin from the arrival times, holds that exactly one option breaks the quick double play, that cover outran the accepted pursuit profile before the decision, that cover radius matches the accepted reach, that the relay still beats the direct throw, that covering now inherits the accepted speed and build-up exactly with a zero read across all three body kinds, and that the boundary and correction stay on the record. No runtime, rules-file, asset, roster, input or UI edits, candidate simulation, standalone build or human gate pass. [Current report](https://github.com/jackguillet/grand-sluggers/blob/9fb972e/docs/research-game-feel-708.md). Both issues remain open.

### Migration questions closed; the CPU dives on the live ball — `3afcfca` / #709

Step 3a. Two of the three questions consolidated and the third went to Jack.

**F693-04-park-migration.** Parks carry only fences, wall height, wind and hazards, and **basepath is not a park property**, so the whole game shares one infield and C80 is a single change rather than six. Every fence scales by **0.70**, with Harbor keeping its accepted 232 / 280 / 232 — a flat 0.70 gives 231 at the poles and that foot is rounding rather than design. Compact dimensions: canopy-yard 218 / 265 / 223, crystal-rink 224 / 270 / 224, ember-keep 237 / 286 / 237, funfair-park 220 / 273 / 238, rooftop-city 223 / 272 / 225. One scale preserves each park's identity and their size order, where re-deriving each from Harbor's 2.9 / 3.5 / 2.9 ratios would flatten all six into the same stadium. **Wall heights do not scale**, since bodies did not shrink and the accepted spatial contract deliberately froze Harbor's 12 feet. **Hazards migrate too**: five parks place barrels, freeze volumes, lava pits, warp pipes, billboards and AC units at explicit x/z straddling the 155-foot infield lip, so they scale by the zone they sit in, per hazard type rather than by a blanket multiply, with their own validator pass. The park change ships in the same commit as the drag change.

**F693-02-arm-rating-migration.** Characters gain `arm`, validated 1 to 10, seeded at their current `field`. **Arm is excluded from `Teams.Tools` through the parity slice**: Tools feeds only `FillScore`, the roster auto-fill ranking, against chemistry at 80 / -40 / 10 and faction at 30, and with Arm equal to Field on day one, including it would double-count defence and change which players the auto-fill picks before any deliberate Arm value exists. Revisit it as a roster decision once real Arm numbers are authored. The HUD and CLI gain an A, which belongs to a presentation child. The CPU reaction group — `InPlay.ThrowReactionSec`, `StealThrow.CpuReleaseSec`, `ClosePlay.CpuReactionSec` — follows Fielding, all three being reaction rather than throwing; whether they should be character-driven at all stays open as a difficulty question.

**A structural finding for the parity slice.** `Diamond.First`, `Second`, `Third` and `Home` are `static readonly` C# constants rather than data. Moving them into `data/` at today's values, changing nothing, is the first commit of the parity slice and the cheapest possible test of R3's migrate-then-prove-parity rule.

**F693-02-cpu-dive-intent-policy — accepted by Jack on September 15, 2026.** Lookahead turned out not to be the question: `FieldingPursuit.Plan` already returns `MissFt`, how many feet short the glove will be, already routes against the complete trajectory for ordinary pursuit, and #640 already supplies the *last makeable moment* commitment idiom. The real problem was that a CPU diving exactly when `MissFt` is inside dive reach would **essentially always reach**, leaving only the handling roll to beat it — the convert-only behaviour Jack explicitly declined, wearing a delay.

Jack chose to decide from the live ball. The CPU dive decision reads **where the ball is and how it is moving at the instant it commits**, not the resolved future path, still committing at the last makeable moment. A ball that changes after the commitment beats the dive — a late bounce, a carom, a deflection off another glove — so **a CPU dive misses because the world moved rather than because a roll said so**, which is what the arcade contract asks for everywhere else. The wrong-footed look that produces is deliberate and can only be judged on screen.
