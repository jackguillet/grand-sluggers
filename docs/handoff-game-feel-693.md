...
**`F693-02-coverage-budget` research is done and one decision is open**, put to Jack on September 15, 2026; context is in `coverageBudgetResearch` and the report. Cover, cutoff and backup all move through `StepFlat` at a flat `cover.ftPerSec` of 28 ft/s with no acceleration, braking, analog shaping or character difference, starting at `max(0.23, the position's reaction lockout)`. Ball pursuit is the accepted 18 ft/s profile with a 0.20-second build-up. So **the same body runs 56% faster to a bag than to a ball**, which `F693-02-pursuit-speed`'s one-profile direction did not intend. Second base is the pressure point at 38 feet from the middle infielders; the corner bags barely move. Applying the accepted profile *with* its read breaks the quick double play by 0.16 seconds, applying it *without* a read does not — and dropping the read is principled, since covering is a known assignment to a fixed spot rather than the recognition of where a batted ball went.

Three things turn out **not** to be at risk and are worth not re-litigating: the cutoff, which repositions while a fly hangs 4.3–8.5 seconds; the relay, which still beats the deepest C80 direct throw 4.00 seconds to 4.80; and receiver reach, since `cover.radiusFt` 6 now matches the accepted 6-foot stand-up reach and has harmonised a long-standing inconsistency by accident — do not split them apart again. One asymmetry is named but not decided: `CpuThrowReadySec` makes the CPU wait for its receiver, while a human can fire at an empty bag, so slowing cover squeezes the player and not the computer. **If no answer is recorded, it is still open — put it to him again rather than choosing for him.**
