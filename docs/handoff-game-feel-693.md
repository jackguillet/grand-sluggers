...
`decisionQueue` now has exactly one `next-human-decision`, **`F693-02-cpu-dive-intent`**, researched and put to Jack on September 15, 2026: the CPU defence reaches the rim only through `AutoDive`, so removing it leaves the CPU with no dive at all, and the dive is worth a flat 16-foot band of every gap. Options are deliberate CPU dive under the same delay, never dive, or dive only on balls it converts. Context is in `cpuDiveIntentResearch`. **If no answer is recorded, it is still open — put it to him again rather than choosing for him.** `F693-02-dive-recovery-cost` is `research-in-progress` in `diveRecoveryCostResearch`, sequenced behind it because a CPU that dives changes what the delay has to be worth; consolidate it into one proposed trial rather than arbitrating coefficients, and only return to Jack if its shape turns out to be a real tradeoff.
