...
`decisionQueue` now has **zero `next-human-decision` entries**. The reach answer immediately raises the next pending row, `F693-02-dive-jump-scoop-reach`: dive, jump and scoop additions are still +8 / +8 / +4 feet in the current runtime, which makes them larger than the accepted stand-up radius and therefore the dominant reach. That is the natural next decision, but research it before asking.
