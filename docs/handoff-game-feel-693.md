...
**`F693-04-flight-budget` research is done and one decision is open**, put to Jack on September 15, 2026; context is in `flightBudgetResearch` and the report. The headline: **C80 as specified is a home-run derby** — a Power-3 bat clears centre on a charged perfect swing, a Power-10 bat homers on ordinary uncharged line-drive contact, and on the control field both of those are comfortably in play. Figures come from the game's own model, cross-checked against `BallFlight.CarryFeet` within 0.7 foot. Two things are worth carrying forward whatever Jack answers. **The doubles engine is the liner, not the fly** — `linerTimeScale 1.0` keeps liners on the real clock at 1.8–3.3 seconds against the 2.56-second alley closure, while flies hang 4.3–8.5 seconds and are always caught; the risk to gap hits is that liners *clear the fence*, not that they get caught. And **the ball is global**: all six parks are still at control scale, so any drag or exit change applies everywhere and every park needs its own compact migration. **If no answer is recorded, it is still open — put it to him again rather than choosing for him.**
