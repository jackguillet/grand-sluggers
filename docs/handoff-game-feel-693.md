...
`decisionQueue` now has exactly one `next-human-decision`: **`F693-02-dive-jump-scoop-reach`**, researched and put to Jack on September 15, 2026. The runtime audit found the dive fires automatically for any fielder the seat is not steering and for every CPU fielder, so 6 feet plus an automatic 8 is 14 feet of passive coverage — more than the 13 just removed, which would make the reach decision cosmetic for eight of nine defenders. The jump is already armed by the West press and the loose-ball scoop is independent of catch radius, so both are out of scope; the dirt scoop pad is deferred behind the dive. Options, ownership evidence and arithmetic are in `diveJumpScoopReachResearch` in the candidate JSON. **If no answer is recorded, it is still open — put it to him again rather than choosing for him, and do not reopen it once an answer is recorded.**
