...
**`F693-02-cpu-dive-intent-policy` was put to Jack and accepted the same day, so 3a is complete.** Lookahead turned out not to be the question — `FieldingPursuit.Plan` already returns `MissFt` and already routes against the complete trajectory for ordinary pursuit, and #640 already supplies the *last makeable moment* commitment idiom. The real problem was that a CPU diving exactly when `MissFt` is inside dive reach would essentially always reach, leaving only the handling roll to beat it, which is the convert-only behaviour Jack declined wearing a delay.

Jack's answer: **the CPU dive decision reads the ball's live position and motion at the instant it commits**, not the resolved future path. A ball that changes after the commitment — a late bounce, a carom, a deflection off another glove — beats the dive, so a CPU dive misses because the world moved rather than because a roll said so. **Boundary: this changes the dive decision only.** Ordinary CPU pursuit keeps `FieldingPursuit.Plan` and its complete-trajectory route and must not be slowed by it, and the CPU still pays the same delay and reaches the same 14 feet as a seat. **The check worth writing first is that a trace run produces real misses; a run with none means the predicate is reading too much.** Do not reopen this.

`decisionQueue` has **zero `next-human-decision` entries**, and the next work is 3b, the parity slice.
