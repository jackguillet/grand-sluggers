...
`data/parks/*.json` — id, name, dimensions, surface, wind, fence height, hazards[]. A hazard is `{ "type": "freeze_volume", "x": ..., "z": ..., "radius": ... }` etc.

**As built (`d0c6e12c`).** An unknown hazard *type* stops the load. An unknown *field* is dropped with no error and no warning: `notes`, `nightOnly`, `dayOnly` and the train's `periodSec` are read by nothing. Nothing is ticked; each hazard is tested once against the ball's landing point (spec §14). Unity draws Harbor from `HarborKit` and every other park from an older primitive diamond plus one `ParkView` method per park id, so "same diamond kit" in the entries above is intent, not fact. A trial root (`trials/c80`) carries its own copy of every park file, key for key.
