...
**Hazards off (F4-h, #858; FD-10).** Every park can be played with hazards off: `cli match --park <id> --hazards off` in the sim today, a title option later (F4-i). The switch removes every hazard instance whose pattern is a status volume, a ball redirect, a reward target or a catch stealer, and keeps everything else the park names — its fence, walls, air, wind, ground, night window, its climb wall and its scenery (spec §14).
