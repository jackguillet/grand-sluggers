...
Where the zones *are* is not this file's business: the lip is `flight.classes.infieldLipFt`, the track is `ParkDiamond.TrackWidth` inside the fence at that bearing, and the chalk is the ±45° foul line. Those are the numbers the field already had and they belong to #730 / #732; `GroundZones.ZoneAt(x, z)` reads them. The wall is the other library (`data/rules/walls.json`, one row: `padded`): a carom reads the row of the material the segment it meets is made of, and every segment is `padded` (`WallMaterial.OfSegment`). Which span of a fence is made of what — and whether it can be climbed or robbed over — comes with the polyline fence (F2-c), not with the material.
