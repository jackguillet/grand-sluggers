...
Requirement added September 19, 2026; runtime catalog/runner implementation is still planned. [tutorials.md](tutorials.md) owns the lesson contract and migration backlog. Every new or changed player-facing mechanic updates its tutorial mapping in the same PR: stable ids, setup and CPU policy, required human command, typed success/failure, retry, profile support, regression evidence and implementation state. Existing broad Practice lessons do not establish complete coverage.

Gameplay children own reusable sim setup, controlled CPU commands, objective evaluators and coverage validation. Presentation children own discovery, coaching, feedback, input-scheme copy and the paired `HowToPlay.cs` / `docs/how-to-play.md` updates. The lesson uses the real game; scripts manufacture an opportunity, never a credited player action or verdict. Planned/blocked coverage must name its owning issue. The future catalog gate must compare lessons to an independent mechanic inventory, so a feature omitted from both a lesson list and its tests is detectable. Human learning and transfer to Exhibition remain Jack's gate.
