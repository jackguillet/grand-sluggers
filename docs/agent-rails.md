...
**Balance runs on demand.** Balance and calibration means: `[Trait("Kind","Balance")]` tests, S-29 and the cohort bands, park-factor reports, flight probes and the evidence seals. Run them when the PR's purpose is tuning or balance, or when Jack asks: Actions → **Full tests** (the `balance_only` input runs only the balance set). A feature PR that moves a feel or rule number does not owe a cohort report. It names the number and the move in the PR body. The balance check runs later.
