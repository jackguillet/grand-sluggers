...
**Recovery composition review, September 15, 2026:** Jack objected to concurrent ordinary/special recovery masking the value of a higher Field rating when the special dominates. The revised pending recommendation adds ordinary and special contributions from the same impact, preserving the ordinary Field-dependent time difference. No composition rule is approved yet; unrelated/repeated effects and per-action permissions remain separate. The prior unaccepted overlap proposal is retained in the research history.
