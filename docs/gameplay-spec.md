...
| `infield.json` | `baselineFt`, `moundFt`, `cornerFt`, `secondFt` — the diamond every park shares, read by `Diamond` (#711). The corners are authored, not derived: a 90-ft baseline rotated 45° is 63.6396…, and the diamond has always played at a rounded 63.64. Outfield depth, the foul area and the park environment are **not** here — those vary per park (#713) |
