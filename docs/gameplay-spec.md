...
A hand-off is **never** triggered by distance alone, never re-evaluated per frame, and **never takes a body that still has a route to the ball** — the SMB4 complaint ("as you near the ball the game swaps it to CF and your guy runs the wrong way") cannot happen because the infield→outfield hand-off requires the current glove's route to have failed: on the ground the glove keeps the ball while its planned route meets it no later than the outfielder's, or the ball is inside its scoop reach. After a hand-off the previous body keeps its momentum for `chase.handoffCoastSec` (0.2 s, the FIFA "move assistance" idea) so the swap does not jerk; the new body answers the stick at once; a throw's release is not a hand-off that coasts (the thrower stays put, §8.5). The stick take is not a re-pick either: it takes the body wearing the ring. ✅ events (`HandGloveTo` call sites), the ground guard and the coast (`TryHandoffOutfield`, `TickHandoffCoast`, S-96, S-97, #633); ⚠️ in the air the plant alone decides — a liner the infield could reach past the lip is still the outfield's, because applying the guard there drops the S-29 band under the stretched hang (#636).
...
| S-97 | Human took SS (Select) on a liner over their head that nobody catches | Ball lands on the grass past them; the stick goes dead | Hand-off to the outfielder by route the first frame the chase runs with SS having no route; SS coasts 0.2 s; nobody teleports |
