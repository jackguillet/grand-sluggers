...
Under F693-03, Jack accepted **quick release and readable travel, with a visibly registering transfer/release** on September 14, 2026. Quick does not mean instant: possession, transfer, release, and follow-through must remain recognizable at ordinary gameplay speed and couch distance. Compare ordinary Wii and GameCube throws to set the timing; special quick-transfer or boosted throws cannot define the ordinary baseline. The separate release-clock decision below selects the ordinary trial duration; travel intervals, curves and numerical reference fidelity remain pending; the provisional visual lead is specified above.
