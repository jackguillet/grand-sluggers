...
- **The zone is the batter's, knee to chest** (CH-06). Its bottom is the knee landmark and its top is the chest landmark of the body the batter wears, at rest, in world feet (`StrikeZoneGeometry.For(batter)`, from `Silhouette.Landmarks`: rig `anatomy.knee` and `anatomy.chest` × the root scale, with every build channel the body has). It scales vertically with the body; horizontally it is the plate, a half-width of **0.92 ft** for every batter.
  On the shipped roster, bottom–top in feet: Zig 0.67–2.22, Fenn 0.79–2.61, Rio 0.96–3.18, Brondo 1.03–3.42, Vale 1.16–3.85, Konga 1.20–3.99, Ashlord 1.24–4.13. A role player stands in its captain's zone.
...
- **Safety net.** `pitching.zone` clamps the knee into `bottomMinFt`–`bottomMaxFt` (0.6–1.45), the chest into `topMinFt`–`topMaxFt` (2.1–4.3), then the height into `heightMinFt`–`heightMaxFt` (1.4–3.0) about the zone's center. It is not a design lever: every shipped batter sits inside it untouched. The height floor keeps S-108 for any body: the lowest family crosses a fixed share of the zone inside the knee, and at 1.4 ft that share is wider than a ball.
