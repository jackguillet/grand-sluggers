...
| Charge | Hold RT to MAX (`pitchChargeSeconds` 0.55), release inside the MAX band (`chargeMaxHoldSeconds` 0.5) | +mph. Released inside the first 0.25 s of MAX (`pitching.release.niceBandSec`) = **Nice!** (+5%, `niceMul`; `PitchCommand.Nice`). Past the band the charge decays (`chargeOverchargeDecay`) toward a normal pitch, never under `match.charge.slapBelow` (0.2). A charged pitch (at or above `match.charge.chargeAt`, 0.55) takes only 10% of the break (`flight.breakDampedMul`; reference: charge and changeup are "essentially straight") ✅ P1 |
...
- **The zone is the batter's, mid-thigh to chest** (CH-06). Its bottom is the mid-thigh landmark and its top is the chest landmark of the body the batter wears, at rest, in world feet (`StrikeZoneGeometry.For(batter)`, from `Silhouette.ZoneLandmarks`: rig `anatomy.thighMid`, halfway from the hip joint to the knee, and `anatomy.chest`, × the root scale, with every build channel the body has). It scales vertically with the body; horizontally it is the plate, a half-width of **0.92 ft** for every batter.
  On the shipped roster, bottom–top in feet: Zig 0.88–2.28 (the height floor below), Fenn 1.09–2.61, Rio 1.33–3.18, Brondo 1.43–3.42, Vale 1.61–3.85, Konga 1.67–3.99, Ashlord 1.73–4.13. A role player stands in its captain's zone.
- **Over the front of the plate, drawn where it is judged.** The zone's plane is the plate's front edge, the pitcher's side (`StrikeZoneGeometry.PlateZ` = `HomeSet.PlateFrontZ`). The flight runs from the release to that plane (`PitchFlight.Point` at u = 1), the umpire judges the crossing there, and the white frame, the aim tell and the cursor are drawn there. The batter's body stands a little further up the box (`HomeSet.BatterZ`), so from the plate camera, above and behind, a height on the plane draws somewhat lower on screen than the same height on the body.
- **The pose never moves it.** The zone is read from the rest rig and the captain's data, never from a take: the stance, the load, the swing and the box walk leave it where it is.
- **One zone per batter.** The same batter has the same zone in 1P and 1v1, from either seat, in either half, in every park. The match stamps each delivery with its batter's zone (`Match.PreparePitch`, `PitchCommand.Zone`), and every reader reads that one zone: the umpire (`StrikeZoneGeometry.Contains`, `AtBatResolver.PitchInZone`), the aim tell and the SET ring (`SetTells`), the cursor and the sweet-spot oval (`SweetSpot`), the CPU pitcher and the CPU batter, and the white frame, which draws the sim's zone and has no numbers of its own.
- **Aim follows the batter.** A pitch aimed at the middle crosses the middle of this batter's zone. The family table's heights (`dropFt`, the vertical aim, a star's rise) are feet of a **reference zone**, 1.45 to 3.65 ft (`StrikeZoneGeometry.Reference`), scaled by the batter's zone height over the reference's (`BatterZone.VerticalScale`).
  So every family keeps its place in every zone: a pitch that is a strike on one batter is a strike on every batter, and the pitch-height, under-the-ball and hit-by-pitch terms (§4.6, §5.4) read the crossing's place in the zone, not raw feet. The reference zone is also the fallback for a pitch no batter faces (a probe or a tool); no live path reads it.
- **Safety net.** `pitching.zone` clamps the mid-thigh into `bottomMinFt`–`bottomMaxFt` (0.6–1.9), the chest into `topMinFt`–`topMaxFt` (2.1–4.3), then the height into `heightMinFt`–`heightMaxFt` (1.4–3.0) about the zone's center. It is not a design lever.
  The height floor keeps S-108 for any body: the lowest family crosses a fixed share of the zone above its bottom, and at 1.4 ft that share is wider than a ball. Zig's thigh-to-chest is 1.29 ft, so his zone keeps its middle and takes the 1.4 ft floor; every other shipped batter sits inside the net untouched.
...
- **The SET ring is the rubber, not the crossing** (PH-06, PH-06-R1). In ordinary play the pale ring on the pitching seat sits at the pitcher's own rubber walked into world feet (`SetTells.RubberRing`: X = `PitcherOffsetX × HomeSet.PitcherWalk`, Y = the middle of the batter's zone), and it **hides at release** — in flight the ball is the cue. It may not be drawn from `PitchFlight.Crossing`:
...
- If the pitch's plate-plane point lies inside the batter's body circle (`batting.hbp.bodyRadiusFt` 0.45, world feet, centered where the batter body actually is, including box walk, at the natural crossing height: the middle of the batter's zone; up and down the circle stretches with the zone, §4.4) **and the batter did not swing**, the batter is hit: first base, forced runners advance, ball dead, stamp HIT BY PITCH. Balls/strikes unchanged. ✅ P1 (S-16, S-17)
...
- **The ARM bar** on the pitcher card drains with the pool and warms with the fade: fresh (white) at `fadeFrom` or more, warming toward amber as the pool falls to `tiredBelow`, red below it with the TIRED word (`BroadcastHud.ArmColor`). The tell grows with the effect; the TIRED word stays the one step, because it is also the CPU's swap.
...
2. **No vertical intent exists.** Height is whatever the family gives (PH-03, §4.2): the crossing is the middle of the batter's zone less the row's `dropFt` scaled to that zone (§4.4), and nothing else in the model touches it. There is no vertical spread (`locations.middleYSpreadFt` went with the endpoint model).
