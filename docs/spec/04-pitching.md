...
- **Drawn where it is judged.** The white frame stands on the plate plane the umpire judges (`StrikeZoneGeometry.PlateZ`, the u=1 crossing), not on the batter's body, which stands 3 ft further up the box (`HomeSet.BatterZ`). From the plate camera, above and behind, a height on that plane draws lower on screen than the same height on the body.
