...
**FLIGHT, What may happen.** Ball travels rubber → plate in `AirSeconds` (≈0.69–1.28 shipped; D7 **wait** — called in the Exhibition sitting before retuning). The floor `pitching.flight.airMinSec` scales with the mound: 0.78 s × 53.78 / 60.5 ≈ 0.69 s, so on the 53.78-ft mound a pitch reaches the floor only above ~109 mph (it was ~96 mph at 0.78 s) and every real pitch keeps its speed difference. Pitcher steers break (stick L/R).
...
- **A CPU pitcher selects by presses too** (§4.8, PH-18-R1). It does not name a family: it rolls its count row's weights over the slots its own repertoire and the active table make *selectable*, and then walks that choice through the same cycle from the same SET reset, so what it throws is always 0, 1 or 2 presses from the fastball. It has no vertical aim either — its location is the rubber it walks to during this SET, and it has all of SET to walk there.
  ✅ (`CpuPitcher.PitchByInputs`, S-115 … S-120). There is no endpoint model and no switch for one.
- **The box recenters after every pitch** (D12); Down recenters early in SET; the pitcher's rubber persists. ✅: every pitch's finish returns through `Match.AfterPitch`, which calls `ResetBatter` (take, swing and miss, foul, strikeout, walk, HBP, ball in play); `BatterContactOffsetX` still latches the walk the swing used (S-16 under D12).
