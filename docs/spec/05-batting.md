...
- **The drawn oval is the judged oval**. One sim function, `SweetSpot.Oval`, returns what the client draws for a swing: the center from the box walk (`WorldCenter`) and the half-extents — tip, handle and the zone's half height — from `SweetSpot.SwingBarrel`, the barrel the resolver judges with (Contact + the bat's `contactMod` clamped 1–10, the charge as it stands with the Charge Bat as a MAX).
