...
Setup-flow coverage: T-G01 and T-G05 begin at stadium and then use captain selection; their accepted roster/order/glove and distinct-seat objectives, CPU policy, retries and profile support are unchanged. Match settings is covered by ExhibitionSettingsTests and LineupScreensTests (ownership, unavailable Items, ready reset and back preservation); a guided settings objective remains planned debt under #958. Standalone two-controller learning remains a human gate.
