...
- **Star lessons and prices:** every Star Pitch and Star Swing lesson (T-P09, T-B09, T-SP-*, T-SS-*, T-X02) and T-G03 must fund the named special at its price on the lesson's own team: the ability's tier price from `stars.json` `tiers`, plus `costs.guestCaptainSurcharge` when the character is a captain playing for another captain's team (§12). `cli tutorials` checks it on the active profile, so a lesson the `trials/stars` prices would leave short is refused there too; the six guest-captain top-tier lessons start at 5 Stars for that reason. A special released without enough Stars is the ordinary action and spends nothing (PH-16-R12); the sim does it today, but no client can release an unaffordable special until the held modifier and its feedback land (P5-c), so its lesson is P5-c's (planned). A lesson's match starts both pools on the one `startingReserve`, as every match does; a setup's `startingStars` / `opponentStars` can only raise its side's pool. T-G03 still earns before it spends: the ordinary strikeout adds its bonus (and, under a trial with a base gain, the plate-appearance base) to a pool that started on the reserve.
