...
1. Discuss the next ready question using a short player-facing example and two or three real alternatives. The broad direction round is complete. Next scoped discussion: **PH-02-R5**, choose the consolidated cycling flow and its between-pitch default. PH-02-R4 selects one button cycling through the three choices. PH-02-R3 locks the chosen pitch family when charging starts. PH-15-R4 accepts all 18 role-player starting assignments. PH-15-R3 selects individually authored repertoires. The seven captain starting repertoires are accepted under PH-15-R2. PH-15-R1 fixes a fastball plus two other pitches for every pitcher. The accepted library is fastball, changeup, curveball, slider and sinker; detailed weaknesses/counterplay and tuning still follow. PH-02-R1 fixes the count at three ordinary pitches per pitcher; charge is a modifier and stars are separate. This refines accepted directions rather than reopening them. PH-14 also retains Jack’s contact-quality follow-up and the proposed answer.
...
**Proposal PH-02-R5 — pending Jack’s choice:** RB / Tab cycles Fastball → alternative 1 → alternative 2 → Fastball, using the accepted repertoire order. South / Space / Enter / left click charges/releases, locking the type at charge start. The shared display can list the three available pitches in order without revealing the active selection. Keep stick movement, D-pad bag/pickoff inputs and live steering. Recommended: reset to Fastball each new pitch, giving a known zero/one/two-tap choice. Alternative: remember the previous selection between pitches, resetting on a pitcher swap. Neither binding nor reset policy is accepted yet; the baseline CyclePitch input property alone does not prove live use.
...
- [ ] Review PH-02-R5 cycling bindings/default/feedback flow; then traits and weaknesses, including the open PH-14 bunt-quality proposal.
