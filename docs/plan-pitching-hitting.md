...
1. Discuss the next ready question using a short player-facing example and two or three real alternatives. The broad direction round is complete. Next scoped discussion: **PH-05-R1**, define how charging trades control for power while preserving live steering and each pitch family’s identity. Repertoires and the cycling flow are recorded under PH-02/PH-15. PH-14 also retains Jack’s contact-quality follow-up and the proposed answer. Refine the accepted directions without reopening them or treating unresolved author proposals as selected.
...
- [ ] Refine PH-05-R1 charge/control behavior, then consolidate remaining pitch/trait/input contracts and the open PH-14 bunt-quality proposal.
