...
**Proposal PH-14-R5 — pending Jack’s choice:** use two directional holds: LT / J for a third-base-side bunt, RT / L for a first-base-side bunt. Change the held side to change intent; if both are held, the most recently pressed side is active. Release all bunt holds to withdraw before contact. Left stick / WASD still positions the batter. Alternative: keep West / V / Left Ctrl as the held bunt action and use LT / J or RT / L only to choose side; its default and selector persistence remain to be specified. Both layouts are proposals. Audit legacy trigger modifiers and phase rearming; reconcile the old bunt verb with paired HowToPlay/docs and diagrams. No new input toolkit or runtime change.
...
- [ ] Review PH-14-R5 bundled bunt bindings, then consolidate swing-cancel/input/trait contracts; compare the tentative fatigue steering effect.
