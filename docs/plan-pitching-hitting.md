...
1. **Define the ordinary pitching duel (PH-02–06, PH-15, PH-18).** Use the accepted three ordinary pitches per pitcher from a shared library; specify each library pitch by readable shape, tactical purpose and weakness, then character assignments and selection inputs. Charge modifies a pitch and stars remain outside the count. Consolidate mound/shape location, live steering, power/control behavior and equal CPU action limits into one reviewable contract. Audit current CPU endpoint targeting before deriving fair controls. Gather required baseline evidence, including the named D7 sitting before any pitch-speed tuning.
...
- [ ] Refine PH-02/PH-15 library membership, pitch purposes/weaknesses and character assignments, then remaining contracts including the PH-14 bunt-quality proposal.
