...
Select a runner with D-pad / 1–3; L3 / Z or pointing toward the next bag starts them immediately, even before the pitch. Point back or use RB / period to return from their current position. Point forward again to reverse. There is no perfect-arm bonus. A small race view shows the departure while the batting camera stays readable; catcher possession flips close behind the catcher, in front of the home scoreboard, looking upfield. Home and centerfield stay vertically aligned. The camera moves gently upfield with the throw; both bags stay visible through returns and rundowns. A pitcher throw uses the same race view.
