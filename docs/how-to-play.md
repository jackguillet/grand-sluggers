...
Contents keeps navigation in the selected scheme: South/East on the controller page, click/Space/Esc on the keyboard page. The F6 input-mode reminder belongs to the keyboard page. Both intros fit the same two-line band at the booklet's readable type size.
...
Throw / swing when the rings line up → **Nice!** / **Nice Hit!**. Late charge is weaker than MAX. The pitch has a **gold streak** so you can see it come in. Start the swing before the ball reaches home: the bat reaches contact 0.30 seconds after South / Space / left click. Timing is judged when the bat comes around. A tap is a full slap swing; only holding West / V / Ctrl bunts. The pitcher releases at the 0.42-second mark of the delivery. Pitcher, batter, and ball share contact slow-down and pause.
