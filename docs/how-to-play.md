...
- **D-pad / 1 2 3 + South** — pickoff before the pitch. A runner on the bag is always safe; only a runner who armed a steal can be caught.
...
- **D-pad / 1 2 3** — select a runner (right 1B, up 2B, left 3B). **Down / 4** is the batter-runner once the ball is live (before the pitch it is home — not stealable). Default highlight is the lead runner.
- **Stick** toward the next bag — before the pitch, arm a **steal** on the highlighted runner (same as L3); back cancels it. Once the ball is live, the same stick **sends** that runner to the next bag and back **returns** them. Runners stand on the bag until contact, a steal, or a send: there is no lead.
