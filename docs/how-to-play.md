...
- **Select / R** — opens the pitcher swap in SET: an in-game window shows the defense diamond, just like position setup. Stick left/right / A-D / d-pad inspects **any fielder**; player 1 can also hover or click a portrait. The card shows PITCH / BAT / FIELD / RUN, throwing hand, pitches and remaining ARM. Teammates show Good / Poor / Neutral chemistry with your pick, and the window names where the old pitcher will move. Select / R or **Put on mound** confirms; East / G or **Cancel** closes without changing the team. Baseball inputs wait while you choose. The new arm brings its own pool; the old pitcher takes the vacated glove. Once per half-inning. When the card reads TIRED (sweat), that is the moment.
