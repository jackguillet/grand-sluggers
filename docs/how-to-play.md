...
SET forks **pitcher view vs batter view**, not pad count. When you pitch, SET is the **mound 3/4** (`mound`) — over the pitcher, rubber in the bottom, batter + catcher + boxes at home in the same frame. You can point at the box you are throwing to without the HUD. When you bat, SET is the **plate 3/4** (`plate`) — beside the batter, looking at the mound. Catcher stays behind the camera. One pad or two, same picture for the same role. Pad 2 does not fork the HUD. Pink/gold charge ring **around the box** on the packed dirt (not a pancake under the feet). When they throw, the camera **cuts** to `pitch`: arm through, ball leaving that hand toward you. ~1s to the plate (Sluggers pace, not MLB 90). After the play, SET returns to that role shot, not the backstop. Home bats the bottom. Scorebug sits top-right; batter card bottom-left; pitcher card bottom-right. Highlight “your” card. Those anchors do not move.
