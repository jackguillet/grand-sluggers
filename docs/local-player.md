...
Builds use a dedicated worktree and the editor version in ProjectVersion.txt. Unity runs as a GUI process to respect the Personal license restriction on batch mode; it consumes the existing PlayerBuildGate request. The owned editor writes build evidence outside its temporary project files and requests its own normal exit after the build. The supervisor waits for shutdown; it does not send termination signals to Unity. If an editor fails to finish or exit, it remains available for inspection and normal Quit. Preview builds also get a fresh build worktree, so the source worktree is not changed by Unity imports.
