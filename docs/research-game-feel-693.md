...
The existing S-29 test is **50 games**, constructed from five captain pairs in both home/away orders using seeds 1–5. It is not fifty distinct seeds and is not Harbor-only: `Match.Exhibition` selects the home captain's park when none is specified. Its current assertions allow mean away and home runs **1.8–5**, doubles fewer than singles, mean homers at most **2 per game**, and nonzero aggregate strikeouts/walks. The spec's headline still says **2–5**, while its #667 note explains the 1.8 floor. Treat this as an explicit unresolved spec/test discrepancy; neither silently lower the headline nor retune gameplay to disguise it.
