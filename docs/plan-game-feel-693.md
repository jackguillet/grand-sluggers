...
**Open decision — F693-02-cpu-dive-intent:** [who dives now that nobody dives for free](research-game-feel-708.md#who-dives-now-that-nobody-dives-for-free). The CPU defence reaches the rim only through `AutoDive`, so removing it leaves the CPU with no dive at all. The dive is worth a flat 16-foot band of every gap, so this sets how hard the computer is to hit against and whether the opposing defence ever makes a highlight catch. Let the CPU dive deliberately under the same recovery delay, never dive, or dive only on balls it converts. Put to Jack on September 15, 2026; no value is selected. Jump, loose-ball scoop and the 4-foot dirt pad are unchanged.
