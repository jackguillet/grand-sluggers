...
**Steps 1 and 2 are complete as of September 15, 2026.** The fielding contract, the ball and covering movement are all decided, and no human decision is pending. What has not happened is any of it running: roughly a hundred accepted anchors exist only as authored arithmetic and none of them has met the others. The sequence below is ordered by that fact, not by queue order, and the implementation plan that follows it front-loads the parts that can be proven to change nothing.

**1. Flight budget — `F693-04-flight-budget`. Done, September 15, 2026.**
See [what C80 does to the ball](research-game-feel-708.md#flight-budget--what-c80-does-to-the-ball). The compact park did not change the ball, and the ball is now wrong: a Power-3 bat clears centre field on a charged perfect swing and a Power-10 bat leaves the yard on ordinary uncharged line-drive contact. The doubles engine, by contrast, is intact — `linerTimeScale 1.0` keeps liners on the real clock against a 2.56-second alley closure while flies hang 4.3 to 8.5 seconds and are always caught, so gap hits come from liners and the risk is that they clear the fence rather than that they get caught. All six parks are still at control scale and drag is global, so this is the game's ball rather than Harbor's. **Accepted September 15, 2026:** [raise the ball's drag](research-game-feel-708.md#accepted-decision--raise-the-balls-drag) from 0.0019 to a 0.0040 trial, leaving the exit table and the 12-foot wall untouched. Ordinary contact stays in the park, a charged squared-up mid-power swing clears the pole but not centre, the biggest bat clears centre, and star swings clear centre reliably. Gap doubles increase, because liners land in front of the fence instead of clearing it. **The drag change cannot ship alone:** drag is global and the other five parks are still at control scale, where 0.0040 would leave no home runs at all, so the ball change and the park migration must land together. Queued as F693-04-park-migration.
...
**2. Coverage budget — `F693-02-coverage-budget`. Done, September 15, 2026.**
...
**3b. The parity slice — nothing changes.** Introduce the Arm rating seeded at each character's Field value and move the three arm consumers onto it; introduce the authored catch-reach property seeded at `10 + 0.6 × Field`; introduce the explicit defensive-quality trait seeded from Field and move retained-ball recoil onto it. Every accepted throw anchor evaluates identically, every reach is unchanged, and **the existing suite must pass with no expectation edited**. That is the parity proof, and it is the only slice where a failing test means the migration is wrong rather than the design being new.

**3c. The compact profile — everything changes, behind a switch.** `RulesTable.Load` already takes a data root and every system takes an optional table, so the compact contract lands as a **selectable profile beside the shipped defaults** rather than as an edit to them: compact park files alongside the existing six, and a rules overlay carrying drag 0.0040, the runner and pursuit speeds, the 6-foot reach, cover on the accepted movement profile, dive ownership and its recovery curve, and the handling error chance with its three outcomes. The control stays loadable so R3's *compare unchanged control A against each candidate with identical inputs and seeds* is a real comparison rather than a memory. **Drag and the park geometry are one commit** — drag is global and the un-migrated parks would have no home runs at all.

**3d. Whole-race validation.** Run the profile through the #702 instrumentation: routine grounder, gap ball, relay, double play, steal. Report effects rather than repairing fixtures with private exceptions. **Expect numbers to move.** Roughly a hundred accepted anchors have never met each other, and several were derived from the same 0.50-second routine margin, so they are coupled by construction. A trial that survives untouched would be more surprising than one that does not.

**3e. Flip the default, then hand off.** Only after 3d reports clean does the compact profile become the shipped contract. Then R4 for presentation and `F693-07-presentation-budget`, and R5 for Jack's sitting. If the accepted dimensions need kit work, that is the separate art child under #188.

**Deliberately outside this sequence.** `F693-02-special-attack-contracts` is large, independent of the ordinary loop and better judged once the loop is playable; the accepted shared recovery rails approve no individual special, so nothing is being inferred while it waits. `F693-02-movement-response` and `F693-02-throw-before-cover` sit behind validation. None of them blocks the sequence.
