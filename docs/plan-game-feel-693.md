...
**Open decision — F693-02-dive-recovery-cost:** [what the dive delay has to be worth](research-game-feel-708.md#what-the-dive-delay-has-to-be-worth). Consolidated from the packet's own anchors: the illustrative routine race leaves a 0.50-second margin, and a dive delay is spent inside it, so "last resort, not spammed" means an unnecessary dive should not still produce the out. Proposed trial 0.60 seconds, the shortest value at which the deterrent is unambiguous, above the 0.40-second handling stun. Caught and missed dives cost the same; the delay and the handling stun overlap rather than adding, which departs from the additive precedent in F693-02-special-recovery-composition and is flagged rather than buried. The open question is whether recovery varies by character — the last natural home for Jack's "unique dive ability" remark — and if so on a curve narrower than the recoil curve, because on the recoil curve the best defender recovers fast enough that an unnecessary dive is still a comfortable out. Put to Jack on September 15, 2026. Jump, loose-ball scoop and the 4-foot dirt pad are unchanged.
