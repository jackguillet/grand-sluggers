...
**3a. Answer the three migration questions.** `F693-02-arm-rating-migration` (schema, seeding, whether Arm joins the `Teams.Tools` roster-building sum, the HUD row, the CPU reaction group), `F693-04-park-migration` (the other five parks and how the drag change is sequenced with them), and `F693-02-cpu-dive-intent-policy` (when the CPU spends a dive, how far ahead it may look, difficulty scaling). Research each first; most of it is consolidation rather than a tradeoff for Jack. `F693-02-throw-before-cover` can wait — it affects a relay edge case, not the main sequence.
