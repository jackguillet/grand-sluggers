...
**Open decision — F693-02-defensive-trait-mapping:** [every job the Fielding number is doing](research-game-feel-708.md#every-job-the-fielding-number-is-doing). The required inventory is done. `Stats.Field` currently drives four distinct jobs: arm (throw speed, comfortable range, accuracy), hands (recoil duration, error chance, dive recovery), reach (already leaving by two accepted decisions) and three CPU-only reaction timings that are difficulty scaling wearing a character stat. The finding is that **the displayed Fielding number is simultaneously the arm rating and the hands rating**, so a cannon-armed catcher with stone hands cannot exist. The accepted throw anchors sit on the arm half and must keep receiving 0.03 per point of arm speed and 5 feet per point of range. Seeding a new Arm rating from today's Field makes the split numerically free on day one. Split Arm out, keep one defensive rating, or split Arm/Hands/Dive three ways. Put to Jack on September 15, 2026. Jump, loose-ball scoop and the 4-foot dirt pad are unchanged.
