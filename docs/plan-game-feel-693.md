...
**Next human choice — pursuit consistency:** the [movement audit](research-game-feel-701-movement.md) shows separate fielding/baserunning formulas plus current hit-class/position pursuit modifiers. Recommend one ordinary pursuit profile per character across hit types and positions, retaining explicit dash/ability/status effects and separate baserunning tuning. This is pending and would supersede the current §8.1 class modifiers only after a complete compact-field calibration; do not just set them to 1. Exact speeds, acceleration, reaction rules, and numerical dimensions are not selected.
