...
**In progress — #708:** C80 remains the accepted lead spatial trial, with authored running/throwing/pursuit, range/readiness fielding, errors/recovery, independent reach, earned dives, Arm/Fielding split, .0040 drag and shared covering movement. The [report](research-game-feel-708.md) and [candidate register](research/game-feel-708-candidates.json) preserve each approval and its scope. The [governing spec](gameplay-spec.md) now marks conflicting older architecture historical. The latest review corrected grounder-invariance and star-homer claims without replacing approved tuning. Isolated production flight probes have run; no complete compact profile has been simulated or played. The steps below separate accepted direction from unresolved implementation mappings, validation and default promotion.
