...
4. **Parks are kits**, not new `ParkView` methods. Harbor is the template (`placed: true`). Other parks wait until Exhibition is the reason people stay (#37). What may be built first is the rail: one field kit with slots per park and a data-driven greybox ([plan-fields.md](decisions/plan-fields.md), FR-13, FR-14). At `d0c6e12c` five parks are still `ParkView` methods; the [presentation map](research/fields-code-map-presentation.md) records it. The kit exists (✅ F6-a #859, PR #863): `FieldKit` draws the diamond, rail and wall for all six parks from the geometry owner, Harbor dresses it, and the five parks' dress is named greybox builders picked by their slots (F6-d, FD-16-R1): stands, props, backdrop and night, never a method chosen by a park id. **Kit slots:** each row of `data/art/parks.json` names every slot in `ParkKitSlots.All` (lawn, dugouts, wall, scoreboard, stands, backdrop, night, light, sky, hazard actors). Each slot is filled by a builder from `ParkKitSlots.Builders` or left `null`, which draws the greybox. `cli art` validates the rows and prints each park's empty slots. Harbor fills the seven pieces `HarborKit` draws. The Harbor kit is chosen by the lawn slot, and each piece of its dress by its own slot, never by a park id. A new builder is a row in `ParkKitSlots.Builders` plus its code. The `light` and `sky` slots are different: they name rows of `data/art/looks.json` (a day and a night, or null to keep the day's), and each kit row carries a `palette` (grass, water, dirt, wall, cap, pole) for its greybox. A new look is a data row, not a `Look.Rig*` method. **Hazards** (F6-d): `data/art/hazard-actors.json` names the toy each hazard type is drawn as (when the park's `hazardActors` slot is `toy-actors`; empty draws a greybox post) and a ring color per acting pattern. Every acting instance draws a flat ring at the disc the sim reads, reach pad and night multiple included, so the drawn size is the played size. A new toy is a row in `HazardActors.Builders` plus its code. **Backdrops and night rigs** (#1156, WD-03 A, WD-10 A): every park but Harbor names `blockout-backdrop` and `night-rig`. `data/art/backdrops.json` lists each park's rough blockout (boxes, cones, cylinders and spheres at a bearing and distance, facing home), which `tools/blender/backdrop_blockout.py` bakes into the row's slot and its Resources copy; `data/art/night-rigs.json` lists its towers, torches, neon and lanterns, whose glow is an unlit colour and never a light (FD-11-R2). `cli art` refuses a shape or piece within 20 ft of the fence, an unknown kind or colour, and a park that names a builder with no row. Rough is enough until Jack sits the greybox (FD-17).
...
7. After a drop: `tools/test-fast.sh <Classes you touched>` must pass and `dotnet run --project src/GrandSluggers.Cli -- art` must still print `OK`. Never run the full test suite locally; CI runs the breakage suite. Character and Harbor kit drops also need [dual stills](screenshot-gate.md) (DCC + in-game) in `scratchpad/stills/`. A look-critic files; Jack passes.
