...
4. **Parks are kits**, not new `ParkView` methods. Harbor is the template (`placed: true`). Other parks wait until Exhibition is the reason people stay (#37). What may be built first is the rail: one field kit with slots per park and a data-driven greybox ([plan-fields.md](plan-fields.md), FR-13, FR-14). At `d0c6e12c` five parks are still `ParkView` methods; the [presentation map](research/fields-code-map-presentation.md) records it.
