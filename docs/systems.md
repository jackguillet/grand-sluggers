...
Player fielding: move the highlighted fielder, catch button, throw-to-base buttons. Dead stick may still run and catch; the throw is a verb (bag + South) when a human is on defense. Buddy Jump is a timed prompt when two good-chem outfielders are under a homer. On contact the camera is one high diamond (behind home, CF at the top) for the whole play.
