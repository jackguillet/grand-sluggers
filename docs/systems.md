...
Control a **named runner**. Default highlight is the lead runner (furthest along). D-pad / 1–3 selects (right 1B, up 2B, left 3B; down is the batter-runner once the ball is live). Runners are bodies on the basepath (spec §9.1): one speed formula for every segment, the batter leaves the box half a second after contact, nobody passes anybody. There are no leads (D1): a runner stands on the bag until contact, a steal, or a send. Stick toward the next bag before the pitch arms a steal on the selected runner (same as L3); stick back cancels it; once the ball is live the stick sends or returns that runner. **LB / `,`** sends everyone; **RB / `.`** returns everyone; both (or `/`) freeze; a tap of the opposite shoulder halts. Forced runners cannot be held. **L3 / Z** arms a steal on the selected runner toward their next bag. They go on the pitch. After a take or swing-and-miss the steal is a **live catcher throw** to the bag (`StealThrow`) — arm 2B (default on a steal of second) and South. Early + beat the runner is caught stealing; late is a stolen base. Dead stick: CPU catcher still guns. Take the stick and you own it. Pickoff stays a named bag + South **before** the pitch: a runner on the bag is always safe; an armed runner can be caught. Walks and strikeouts cancel. Fair contact always sends the batter to first. On a fly, every runner returns to the bag until the catch or the drop; all-advance before the catch is tag-and-go. The play ends (Time) when three are out, or an infielder holds the ball unthrown and every live runner has stood on a bag for a second. Runs count when the body crossed home before the third out (not on a force). Mini diamond shows occupied bags. No steal home in this pass.
