...
Control a **named runner**. Default highlight is the lead runner (furthest along). D-pad / 1–3 selects (right 1B, up 2B, left 3B; home is not stealable). Stick toward the next bag arms a steal on the selected runner (same as L3); stick back returns and cancels it. There is no lead stick (spec §9.2); LB all-advance still walks a lead until P3 retires leads. **LB / `,`** sends everyone; **RB / `.`** returns everyone; both (or `/`) freeze. **L3 / Z** arms a steal on the selected runner toward their next bag. They go on the pitch. More lead = a better jump and more pickoff risk on a take. After a take or swing-and-miss the steal is a **live catcher throw** to the bag (`StealThrow`) — arm 2B (default on a steal of second) and South. Early + beat the runner is caught stealing; late is a stolen base. Dead stick: CPU catcher still guns. Take the stick and you own it. Pickoff stays a named bag + South **before** the pitch. Walks and strikeouts cancel. Fair contact always sends the batter to first. On a fly, runners hold unless all-advance is on (then they tag up). Mini diamond shows leads. No steal home in this pass.
