...
public class FeelInfraTests
{
    ...
    public void SetTellsAreProductStateNotF2()
    {
        ...
        var mid = SetTells.Locator(new PitchCommand("fastball", 0, 0, false));
        var inRight = SetTells.Locator(new PitchCommand("fastball", 0, 0, false, RubberX: 0.4));
        Assert.True(inRight.X > mid.X);
        Assert.Equal(PitchFlight.Crossing(new PitchCommand("fastball", 0, 0, false, 0.4, -0.2)),
            SetTells.Locator(new PitchCommand("fastball", 0, 0, false, 0.4, -0.2)));
        Assert.True(SetTells.InZone(new PitchCommand("fastball", 0, 0, false)));
        Assert.True(SetTells.InZone(new PitchCommand("fastball", 0, 0, false, 0.4, 0.2)));
        Assert.False(SetTells.InZone(new PitchCommand("fastball", 0, 0, false, 1, 1)));
    }
