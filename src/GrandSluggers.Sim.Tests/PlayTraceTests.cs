...
public sealed class PlayTraceTests
{
    ...
    public void GrounderTrace_GloveMeetsBallAndRunnerVsBagIsTheOut()
    {
        ...
        var takeFt = FieldingResolver.CatchWindowFt(FieldingResolver.CatchRadiusFt(preview.Fielder, match.Park, match.Rules)
        ...
        Assert.True(inGlove.Ball.Held && Diamond.Dist(inGlove.Glove.X, inGlove.Glove.Z, inGlove.Ball.X, inGlove.Ball.Z) < 1,
            $"the held ball is at the glove; glove ({inGlove.Glove.X:0.0},{inGlove.Glove.Z:0.0}) ball ({inGlove.Ball.X:0.0},{inGlove.Ball.Z:0.0})");
    ...
    static AtBatResult Grounder(AtBatResult hit, Match match, double sprayDeg) =>
        Shape(hit, match, exit: 85, launch: -12, spray: sprayDeg);
