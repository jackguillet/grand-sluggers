...
public sealed class HazardPlacementTests
{
    ...
    public void SF23_EveryHazardClearsTheLanesPadsMoundAndPlate()
    {
        ...
        // Every hazard of all six parks was measured, not just the ones FD-19-R1 moved.
        Assert.Equal(26, measured);

        // Finding 31, measured: the breath at its night disc.
        var breath = Game.Parks[ParkIds.Ember].Hazards.Single(h => h.Type == HazardType.FireBreath);
    ...
    public void SF23_AVolumeOnALanePadMoundOrPlateIsRefusedByName()
    {
        ...
        fixture.Park(ParkIds.Crystal, json =>
        {
        ...
        var file = fixture.ParkFile(ParkIds.Crystal);
    ...
    public void SF23_TheDiscIsTheHazardsOwnRadius()
    {
        ...
        var funfair = content.Parks[ParkIds.Funfair].Hazards.Count;
        ...
        (string Park, int Index, string What, double By)[] padFindings = [(ParkIds.Canopy, 2, "the first-second lane", 1.98)];
    ...
    public void FD19R1_TheFiveVolumesStandWhereTheTableSays()
    {
        ...
        [
            (ParkIds.Crystal, 0, "freeze_volume", 47, 83, 5.6),
            (ParkIds.Crystal, 1, "freeze_volume", -44, 86, 5.6),
            (ParkIds.Crystal, 2, "freeze_volume", 7, 131, 7),
            (ParkIds.Ember, 0, "lava_pit", 44, 89, 7),
            (ParkIds.Ember, 1, "lava_pit", -40, 92, 7)
        ];
        ...
        Assert.Equal(3, Game.Parks[ParkIds.Crystal].Hazards.Count);
        Assert.Equal(
            [("lava_pit", 8.0, 132.0, 8.4), ("fire_breath", 0.0, 175.0, 11.2), ("statue", 0.0, 217.0, 7.0)],
            Game.Parks[ParkIds.Ember].Hazards.Skip(2).Select(h => (h.Type, h.X, h.Z, h.Radius)));
    }
