...
public sealed class HazardsOffTests
{
    ...
    static IReadOnlyList<(string Park, bool Night, int Seed)> NoHazardEventSeeds =>
        [
            (ParkId.Crystal, false, 1),   // a freeze volume slows the chase (a Crystal night is its day since FD-11-R2)
            (ParkId.Ember, true, 1),      // a lava pit or the breath slows the chase
            (ParkId.Funfair, true, 16),   // a chomper or a can redirects the ball
            (ParkId.Canopy, false, 4),    // a barrel redirects the ball
            (ParkId.Rooftop, false, 2)    // a billboard pays the batting team
        ];

    /// <summary>A park, a condition and a seed at which the switch changes the whole game.</summary>
    static (string Park, bool Night, int Seed) DefaultOnGame => (ParkId.Canopy, false, 4);
    ...
    public void SF24_AHumanSeatAndTheCpuPlayTheSameHazardsOffPark()
    {
        ...
        foreach (var hazards in new[] { true, false })
        {
            ...
            var cpu = Match.Slice(Catalog, seed: 1, parkId: ParkId.Crystal);
            if (!hazards) cpu = new Match(Catalog, PresetTeams.EmberCourt(Catalog), PresetTeams.SparkAllStars(Catalog),
                Catalog.MustPark(ParkId.Crystal), seed: 1, hazards: false);
