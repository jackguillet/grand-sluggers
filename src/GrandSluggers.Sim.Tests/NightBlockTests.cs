...
public sealed class NightBlockTests
{
    ...
    /// seed 22, trial seed 13, the first night seed after 1 with a chomp), so a mouth that moved, resized or
    ...
    /// the slow (FD-14). P2-e (#891) re-recorded all eight: the plate has no chemistry and the items are dormant,
    /// so every at-bat can change, and the old chomp seeds no longer chomp.
    /// </summary>
    static IReadOnlyList<(string Park, int Seed, string Final, string Sha)> Before => TestRoot.Pick<IReadOnlyList<(string, int, string, string)>>(
        ...
        [
            ...
            ("funfair-park", 13, "Final  Ember Court 7  Spark All-Stars 2", "c9eed702a3f88221ec211acd91a210f4fb1037ce9cbeb7ec80a29283feba95cf"),
