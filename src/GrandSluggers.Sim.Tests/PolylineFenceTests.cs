...
public sealed class PolylineFenceTests
{
    ...
    public void SF06_EveryCatalogParkPlaysAndDrawsTheThreePostFenceBitForBit()
    {
        ...
        Assert.Equal([ParkIds.Crystal], catalog.Parks.Values.Where(p => p.Fence is not null).Select(p => p.Id));
    ...
    {
        var catalog = Game;
        var park = Fenced(catalog.Parks[ParkIds.Harbor]);
    ...
    {
        var catalog = Game;
        var arc = catalog.Parks[ParkIds.Harbor];
        var park = Fenced(arc);
    ...
    public void SF07_AValidFenceBlockLoadsAndThePointsReachThePark()
    {
        ...
        var loaded = ContentCatalog.Load(new DataRoot(shipped.Root)).Parks[ParkIds.Harbor];
    ...
    public void SF12_EachSpanAsksTheLibraryForItsOwnMaterialsRow()
    {
        ...
        var catalog = Game;
        var rules = catalog.Rules;
        var park = Fenced(catalog.Parks[ParkIds.Harbor], halves);
        var polygon = FieldBounds.Of(park);
    ...
    {
        var arc = Game.Parks[ParkIds.Harbor];
