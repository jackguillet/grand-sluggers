...
| --- | --- |
| `rules/batting.json` | `cpu.commitRead` `false` → `true`. Every other field is the shipped file, byte for byte (S-143). |
...
commit therefore beats the read (S-141). The switch adds no draw.
