...
- Falsify with `dotnet test`, `dotnet run --project src/GrandSluggers.Cli -- art`, `cli protocol`, `cli stills`, `cli match` (`--trace` for tick JSON), `tools/unity-compile.sh`. Look/character: DCC still + in-game still in `scratchpad/stills/` in the PR (`tools/dcc-still.sh`, `tools/still-gate-character.sh`). A critic files; Jack passes. Personal Unity cannot `-batchmode`.
