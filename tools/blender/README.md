...
Fill an existing slot (`wall-ring`, `bag`, `home-plate`, …); do not start a second park pipeline or Unity-only park art. Numbers stay in Sim (`HarborWall`, `ParkDiamond`, `HarborDugout`) and data. The script reads them (`tools/data_json.py`, `sim_consts`); the few still copied are pinned by a test that catches drift. The script is the source: interactive work in a Blender MCP session (Blender GUI with its MCP add-on running) ends as an edit to `harbor_kit.py`, not a `.blend`.
